...
'''
# import sys
# sys.path.append('../../CryptoClassification')
...
celery_app = Celery('tasks', broker='redis://:feifei@122.51.255.207:6379/2',
                    backend='redis://:feifei@122.51.255.207:6379/3')
...
def cipher_task(**kwargs):
    ...
    try:
        ...
        #              save_mode='para')
        return_result, time_format = getCipherResult(file_dir='', model_name=kwargs.get('classifier'), args=args1)
        ...
        # try:
        with open(file='static/CipherResult.json', mode='w', encoding='utf-8') as f:
            # 将数据写入json文件
            json.dump(Result, f)

        with open(file='static/ciphertimeRecord.json', mode='w', encoding='utf-8') as f:
            # 将分类时间写入json文件
...
def modelTrain_task(**kwargs):
    ...
    try:
        ...
        #     'accuracy': accuracy
        modelTrainResult = getTrainedModel(file_dir='static/trainModelFile/', model_name=kwargs.get('classifier'),
                                           args=args1)
        # todo: 将训练模型的pkl模型文件以及模型效果图保存至压缩包中，获取modelTrainResult中的两文件地址信息
        with zipfile.ZipFile('static/trained_model.zip', 'w', zipfile.ZIP_DEFLATED) as zf:
            # zf = zipfile.ZipFile('test.zip', 'w', zipfile.ZIP_DEFLATED)
            # 这里如果不指定arcname的话压缩文件中文件会自动加上file_src的路径
            model_filePath = 'self_model/' + kwargs.get('classifier') + '/' + modelTrainResult.get('model_dir')
            img_filePath = 'verify_result/' + kwargs.get('classifier') + '/' + modelTrainResult.get('pic_dir')
            zf.write(filename=model_filePath, arcname=modelTrainResult.get('model_dir'))
            zf.write(filename=img_filePath, arcname=modelTrainResult.get('pic_dir'))

        # 模型训练完成后发送提示信息给用户，
        emailSendObject = emailSend_Object(receivers=[userEmail], receiver_name=kwargs.get('username'))
        emailSendObject.send_file(filepath='static/trained_model.zip', fileName='trained_model.zip',
