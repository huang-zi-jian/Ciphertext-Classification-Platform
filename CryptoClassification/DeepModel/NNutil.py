...
import time  # 在AlexNet和SimpleCNN里面用到了
...
def save_train_result(model_name, crypto_list: list, loss: list, acc: list):
    ...
    with open('/home/ubuntu/CryptoClassification/verify_result/' + model_name + '/' + filename + '_loss.txt', encoding='utf-8', mode='w') as l:
        l.writelines(str(o) + '\n' for o in loss)
    with open('/home/ubuntu/CryptoClassification/verify_result/' + model_name + '/' + filename + '_acc.txt', encoding='utf-8', mode='w') as a:
        a.writelines(str(o) + '\n' for o in acc)
...
def save_verify_result(model_name, crypto_list: list, result):
    ...
    cl = concat2str(crypto_list)
    filename = model_name + '_' + cl
    with open('/home/ubuntu/CryptoClassification/verify_result/' + model_name + '/' + filename + '_result.txt', encoding='utf-8', mode='w') as f:
        f.writelines(str(o) + '\n' for o in result)
...
def plot_fig(model_name, crypto_list: list, epoch: int, loss: list, acc: list):
    ...
    plt.savefig('/home/ubuntu/CryptoClassification/verify_result/' + model_name + '/' + pic_name)
    plt.clf()
    return pic_name
