...
class AlexNet(nn.Module):
    ...
    def __init__(self, num_classes, grayscale=False):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(in_channels, 96, kernel_size=11, padding=1),  # 卷积核96个大小为 11*11
...
def AlexCNN(crypto_list: list, feature_file_dirs: list, input_channel: int, ratio=0.8, epoch=30, batch=100,
    ...
    if save_mode:
        path = '/home/ubuntu/CryptoClassification/'
        torch.save(model, '/home/ubuntu/CryptoClassification/self_model/AlexNet/' + model_name + '_model.pkl')  # todo
        model_name = model_name + '_model.pkl'
    else:
        torch.save(model.state_dict(), '/home/ubuntu/CryptoClassification/self_model/AlexNet/' + model_name + '_parameter.pkl')  # todo
...
if __name__ == '__main__':
    AlexCNN(['3DES', 'AES', 'Blowfish'],
            ['../static/feature/3DES_ones.csv', '../static/feature/AES_ones.csv', '../static/feature/Blowfish_ones.csv'],
            input_channel=1024, loss_function='CE', optimizer='Adam', col_name=['data_frame'], batch=1500, epoch=18)
